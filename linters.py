...
class GraphqlSchemaLint(Linter):
    ...
    # The list of supported rules is at
    #   https://github.com/cjoudrey/graphql-schema-linter
    # I list them all and just comment out the ones we don't want to use.
    _RULES = (
    ...
    def process(self, f, contents_of_f):
        ...
        # We also need to define the directives that federation
        ...
        # have a query, which is fine in federation-land.  We
        # add one if need be.
        if "type Query" not in contents_of_f:
            contents_of_f += ('\n\n"""\nfake\n"""\ntype Query {\n' +
                              '"""fake"""\nfake: String }')

        (stdout, stderr) = p.communicate(input=contents_of_f)
        stdout, stderr = stdout.decode('utf-8'), stderr.decode('utf-8')
