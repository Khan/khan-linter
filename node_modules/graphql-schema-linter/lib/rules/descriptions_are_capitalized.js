...
function DescriptionsAreCapitalized(configuration, context) {
  return {
    FieldDefinition(node, key, parent, path, ancestors) {
      ...
      if (typeof description !== 'string' || description.length === 0) return; // It's possible there could be some markdown characters that do not
      ...
      context.reportError(new _validation_error.ValidationError('descriptions-are-capitalized', `The description for field \`${parentName}.${fieldName}\` should be capitalized.`, [node]));
    }
